...
interface EditDiscountModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: UpdateStudentDiscountInput) => void;
...
export function EditDiscountModal({
  ...
  useEffect(() => {
    if (isOpen && discount) {
      ...
      setShowCategorySelection(Boolean(discount.applicableCategories?.length));
    }
  }, [isOpen, discount]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: UpdateStudentDiscountInput = {
      ...formData,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-xl border-1 border-gray-200 z-10 relative">
        <h3 className="text-lg font-medium mb-4">Edit Student Discount</h3>
